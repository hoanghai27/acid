...
%% Created on 5:46:36 PM Feb 17, 2015
%% Purpose

%% maintainer

-module(acid_tcp_acceptor).
...
-define(ACID_ACCEPTOR_TIMEOUT,1000).

start_link(Args) ->
	proc_lib:start_link(?MODULE,new_acceptor,[erlang:append_element(Args,self())]).

new_acceptor({Type,Ips,Port,Opts,Parent}) when Port > 0 andalso Port < 65536 ->
	IpAddrs = reformat_ip(Ips, []),
	process_flag(trap_exit,true),
	N = get_opt(n_acceptor,Opts,2),
	if
		length(IpAddrs) > 0 ->
			case gen_tcp:listen(Port,[binary,{reuseaddr,true},{packet,tpkt},{active,false}] ++ [{ip,X} || X <- IpAddrs]) of
				{ok,LSocket} ->
					Slaves = acceptor_slave_spawn(N,LSocket,Type,Opts,[]),
...
acceptor_slave_spawn(1,LSocket,Type,Opts,In) ->
	Pid = spawn_link(?MODULE,acceptor_slave_init,[self(),LSocket,Type,Opts]),
	[Pid | In];

acceptor_slave_spawn(N,LSocket,Type,Opts,In) ->
	Pid = spawn_link(?MODULE,acceptor_slave_init,[self(),LSocket,Type,Opts]),
	acceptor_slave_spawn(N -1,LSocket,Type,Opts,[Pid | In]).


acceptor_slave_init(Master,LSocket,Type,Opts) ->
	process_flag(trap_exit, true),
	acceptor_slave(Master,LSocket,Type,Opts).

acceptor_slave(Master,LSocket,Type,Opts) ->
	case gen_tcp:accept(LSocket,?ACID_ACCEPTOR_TIMEOUT) of
		{ok,Socket} ->
			lager:debug("new socket connected from ~p",[peer_info(Socket)]),
			case acid_tcp_sup:add_socket(Type, Socket,Opts) of
				{ok,Pid} -> 
					lager:debug("~p create new controller ~p <-> ~p",[self(),Pid,Socket]),
					gen_tcp:controlling_process(Socket,Pid),
					inet:setopts(Socket,[binary,{active,once},{packet,tpkt}]);
				_ ->
					lager:error("can not create new controller => close new socket"),
					gen_tcp:close(Socket)
			end,
			acceptor_slave_check(Master,LSocket,Type,Opts);
		{error,timeout} ->
			acceptor_slave_check(Master,LSocket,Type,Opts);
		{error,system_limit} ->
